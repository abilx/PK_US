  ...
  @component('components.lurah.breadcrumb')
    @slot('breadcrumb_title')
      <h3>
        Detail Kegiatan</h3>
    @endslot
    <li class="breadcrumb-item"><a href="{{ route('lurah.kegiatan.index') }}">Kegiatan</a></li>
            ...
            <div class="card">
              ...
              <div class="card-footer text-end">
                ...
                <button class='btn @php echo $kegiatan->status_kegiatan == 0 ? 'btn-success' : 'btn-warning' @endphp
                  ...
                  href="{{ route('lurah.kegiatan.edit', $kegiatan->id_kegiatan) }}"><span class="fa fa-edit"></span>
                  {{ $kegiatan->status_kegiatan == 0 ? 'Aktif' : 'Non-Aktif' }}</button>
                {{-- END TOMBOL AKTIF NON AKTIF --}}

                {{-- TOMBOL EDIT --}}
                <a class="btn btn-secondary btn-lg"
                  href="{{ route('lurah.kegiatan.edit', $kegiatan->id_kegiatan) }}"><span
                    class="fa fa-edit"></span>Edit</a>
                {{-- END TOMBOL EDIT --}}

                {{-- TOMBOL DELETE --}}
                <form method="POST" action="{{ route('lurah.kegiatan.destroy', $kegiatan->id_kegiatan) }}"
                  class="d-inline">
                  @csrf
                  @method('DELETE')
  ...
  <script>
    $('#ubah_status').click(function() {
      ...
      $.ajax({
        type: "GET",
        dataType: "json",
        url: "{{ route('lurah.kegiatan.update.status') }}",
        data: {
