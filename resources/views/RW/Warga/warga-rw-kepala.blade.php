...
@section('container')
  @component('components.r-w.breadcrumb')
    @slot('breadcrumb_title')
      <h3>Warga</h3>
    @endslot
    <li class="breadcrumb-item"><a href='{{ route('rw.warga.index') }}'></a>Warga</li>
    <li class="breadcrumb-item active"><a href='{{ route('rw.warga.wargakepala') }}'></a>Daftar Kepala Keluarga</li>
  @endcomponent
  <!-- Form Tambah Warga -->
              ...
              <div class="col-3">
                <div class="bookmark">
                  <a class="btn btn-primary btn-lg"
                    href="{{ route('rw.warga.create') }}" data-bs-original-title="" title="">
                    <span class="fa fa-plus-square"></span>Tambah Data
                  </a>
                </div>
              </div>
            </div>
                ...
                <tbody>
                  @foreach ($warga as $dw)
                    <tr>
                      ...
                      <td>{{ $dw->nik }}</td>
                      <td>{{ $dw->nama_kepala_keluarga }}</td>
                      <td>{{ $dw->alamat }}</td>
                      <td>{{ $dw->no_hp_warga }}</td>
                      <td class="aksi">
                        <a class="btn btn-info btn-sm p-2"
                          href="{{ route('rw.warga.show_warga',$dw->id_warga) }}"><span class="fa fa-eye"></span></a>
                        <a class="btn btn-secondary btn-sm p-2"
                          href="{{ route('rw.warga.edit', $dw->id_warga) }}"><span class="fa fa-edit"></span></a>
                        <form method="POST"
                          action="{{ route('rw.warga.destroy', $dw->id_warga) }}"
                          class="d-inline">
                          @csrf
                          <input name="_method"
                            type="hidden"
