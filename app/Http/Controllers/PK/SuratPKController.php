...
class SuratPKController extends Controller
{
    ...
    {
        $surat = Surat::where('pk', auth()->user()->id_pk)
            ->where('status_tandatangan', 1)
            ->orWhere('status_tandatangan', 2)
            ->latest()
            ->get();
        return view('PetugasKelurahan.surat.surat', [
            'surat' => $surat,
    ...
    public function nomorsurat()
    {
        ...
        $surat = Surat::with('wargas')->where('pk', auth()->user()->id_pk)
        ...
        // dd($surat);
        // return $surat;
        return view('PetugasKelurahan.surat.nomor_surat', [
            'surat' => $surat,
            'title' => "Nomor Surat Petugas Kelurahan"
        ]);
    }
    ...
    public function detailSuratKeterangan(Surat $surat)
    {
        $surat = $surat;
        $title = "Pengajuan Surat";
        return view('PetugasKelurahan.surat.surat_keterangan_detail', compact('surat', 'title'));
    }
    public function prosesSurat(Request $request, Surat $surat)
    {
        ...
        if ($proses == 'terima') {
            ...
            return redirect()->route('pk.surat.index')->with('success', 'Surat telah disetuji!');
        } elseif ($proses == 'tolak') {
            $this->tolakSuratKeterangan($surat);
            return redirect()->route('pk.surat.index')->with('success', 'Surat ditolak!');
        }
    }
    ...
    public function printSuratKeterangan(Surat $surat)
    {
        ...
        if (!$surat and $surat->status_surat == 0) {
            return redirect()->route('pk.surat.index')
    ...
    public function cekSurat()
    {
        ...
        return view('pk.surat.cek_surat');
