...
class Warga extends Authenticatable
{
    ...
    {
        //hasMany(namamodel, foreign key tabel warga, primary key tabel sendiri)
        return $this->belongsTo(PK::class, 'id_warga', 'id_Warga' );
    }
    ...
    {
        return $this->attributes['age'] = $this->calculateAge();
    }

    private function calculateAge()
    {
        $diffInYears = $this->tgl_lahir->diffInYears(now());
        return $diffInYears;
    }

    public function scopeAgeGreaterThan60($query)
    {
        return $query->select('*')
            ->selectRaw("TIMESTAMPDIFF(YEAR, tgl_lahir, CURDATE()) as age")
            ->havingRaw("age > 60");
    }
