...
class WargaRWTest extends TestCase
{
    ...
    {
        $response = $this->get('/RW/warga');

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.warga');
    }

    public function testListWarga()
    {
        $warga = Warga::factory()->create();
        $response = $this->get("/RW/list-rt/{$warga->rw}");

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.tabel_warga');
    }

    public function testWargaKepala()
    {
        $response = $this->get("/RW/warga/wargakepala");

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.warga-rw-kepala');
    }

    public function testWargaMiskin()
    {
        $warga = Warga::factory()->create();
        $response = $this->get("/RW/warga/wargamiskin");

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.kemiskinan.tabel_kemiskinan');
    }

    public function testWargaLansia()
    {
        $response = $this->get("/RW/warga/wargalansia");

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.warga-rw-lansia');
    }

    public function testCreateWarga()
    {
        $response = $this->get('/RW/warga/tambah');

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.warga-tambah-rw');
    }
    ...
    public function testStoreWarga()
    {
        ...
        $response = $this->post('/RW/warga', $data);

        $response->assertStatus(302)
            ->assertRedirect(route('rw.warga.index'))
    ...
    public function testEditWarga()
    {
        ...
        $response = $this->get("/RW/warga/{$warga->id_warga}/edit");

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.warga-edit-rw')
    ...
    public function testUpdateWarga()
    {
        ...
        $response = $this->put("/RW/warga/{$warga->id_warga}", $data);
    ...
    public function testDestroyWarga()
    {
        ...
        $response = $this->delete("/RW/warga/{$warga->id_warga}");
    ...
    public function testShowWarga()
    {
        ...
        $response = $this->get("/RW/warga/detail/{$warga->id_warga}");

        $response->assertStatus(200)
            ->assertViewIs('RW.Warga.detail_warga')
            ->assertViewHas('warga', $warga);
    }
}
